...
#define TOP_LEFT 0, 0, 9, 9
#define TOP_RIGHT dimension - 8, 0, 8, 9
#define BOTTOM_LEFT 0, dimension - 8, 9, 8
#define ALIGNMENT_PATTERN alignmentPatternCenters_[y] - 2, i, 5, 5
#define VERTICAL_TIMING 6, 9, 1, dimension - 17
#define HORIZONTAL_TIMING 9, 6, dimension - 17, 1
#define VERSION_TOP_RIGHT dimension - 11, 0, 3, 6
#define VERSION_BOTTOM_LEFT 0, dimension - 11, 6, 3
...
namespace
{
    ...
    public:
        static VersionHolder const & instance()
        {
            static VersionHolder singleton;
            return singleton;
        }

        auto const & versions()  const noexcept { return VERSIONS;                              }
        auto         nVersions() const noexcept { return static_cast< int >( VERSIONS.size() ); }
    };
...
pping::Fallible<Version *> Version::getProvisionalVersionForDimension(int dimension) MB_NOEXCEPT_EXCEPT_BADALLOC {
  if (dimension % 4 != 1)
    return failure<ReaderException>("Dimension must be 1 mod 4");

  auto const version(getVersionForNumber((dimension - 17) >> 2));
  if(!version)
...
pping::Fallible<Version *> Version::getVersionForNumber(int versionNumber) MB_NOEXCEPT_EXCEPT_BADALLOC {
  if (versionNumber < 1 || versionNumber > VersionHolder::instance().nVersions() )
    return failure<ReaderException>("Version number must be between 1 and 40");

  return VersionHolder::instance().versions()[versionNumber - 1];
}
...
Fallible<Version *> Version::decodeVersionInformation(unsigned int versionBits) MB_NOEXCEPT_EXCEPT_BADALLOC {
  ...
  return failure<ReaderException>("Couldn't decode version information");
}

FallibleRef<BitMatrix> Version::buildFunctionPattern() {
  int dimension = getDimensionForVersion();
  Ref<BitMatrix> functionPattern(new BitMatrix(dimension));

  {
    // Top left finder pattern + separator + format
    auto const & trySetRegion(functionPattern->setRegion(TOP_LEFT));
    if(!trySetRegion)
      return trySetRegion.error();
  }
  {
    // Top right finder pattern + separator + format
    auto const & trySetRegion(functionPattern->setRegion(TOP_RIGHT));
    if(!trySetRegion)
      return trySetRegion.error();
  }
  {
    // Bottom left finder pattern + separator + format
    auto const & trySetRegion(functionPattern->setRegion(BOTTOM_LEFT));
    if(!trySetRegion)
      return trySetRegion.error();
  }
  // Alignment patterns
  ...
  for (size_t x = 0; x < max; x++) {
    ...
    for (size_t y = 0; y < max; y++) {
      ...
      auto const & trySetRegion(functionPattern->setRegion(ALIGNMENT_PATTERN));
      if(!trySetRegion)
        return trySetRegion.error();
    }
  }
  {
    ...
    auto const & trySetRegion(functionPattern->setRegion(VERTICAL_TIMING));
    if(!trySetRegion)
      return trySetRegion.error();
  }
  {
    // Horizontal timing pattern
    auto const & trySetRegion(functionPattern->setRegion(HORIZONTAL_TIMING));
    if(!trySetRegion)
      return trySetRegion.error();
  }
  if (versionNumber_ > 6)
  {
    ...
    auto const & trySetRegionTop(functionPattern->setRegion(VERSION_TOP_RIGHT));
    if(!trySetRegionTop)
      return trySetRegionTop.error();

    // Version info, bottom left
    auto const & trySetRegionBottom(functionPattern->setRegion(VERSION_BOTTOM_LEFT));
    if(!trySetRegionBottom)
      return trySetRegionBottom.error();
