...
#include <zxing/common/detector/math_utils.h>                         // for distance, round, math_utils
...
#include <zxing/qrcode/detector/ZXingQRCodeAlignmentPatternFinder.h>  // for AlignmentPatternFinder
#include <zxing/qrcode/detector/ZXingQRCodeDetector.h>
...
FallibleRef<DetectorResult> Detector::detect(DecodeHints const& hints) MB_NOEXCEPT_EXCEPT_BADALLOC {
  callback_ = hints.getResultPointCallback();
  FinderPatternFinder finder(image_, hints.getResultPointCallback());

  auto const info(finder.find(hints));
  if(!info)
      return info.error();

  if (callback_ != NULL){
    callback_->finishedFindingPoints();
  }
  return processFinderPatternInfo(*info);
}

FallibleRef<DetectorResult> Detector::processFinderPatternInfo(Ref<FinderPatternInfo> info){
  ...
  if (moduleSize < 1.0f) {
      return failure<ReaderException>("Bad module size");
  }

  auto const dimension(computeDimension(topLeft, topRight, bottomLeft, moduleSize));
  if(!dimension)
      return dimension.error();

  auto const provisionalVersion(Version::getProvisionalVersionForDimension(*dimension));
  if(!provisionalVersion)
      return provisionalVersion.error();
  ...
  if ((*provisionalVersion)->getAlignmentPatternCenters().size() > 0) {
    ...
    for (int i = 4; i <= 16; i <<= 1) {
        ...
        auto const foundAlignmentPattern(findAlignmentInRegion(moduleSize, estAlignmentX, estAlignmentY, (float)i));
        if(foundAlignmentPattern) {
            alignmentPattern = *foundAlignmentPattern;
            break;
        }
    }
  }
  ...
  auto const bits(sampleGrid(image_, *dimension, transform));
  if(!bits)
      return bits.error();
...
FallibleRef<BitMatrix> Detector::sampleGrid(Ref<BitMatrix> image, int dimension, Ref<PerspectiveTransform> transform) MB_NOEXCEPT_EXCEPT_BADALLOC {
  GridSampler &sampler = GridSampler::getInstance();
  return sampler.sampleGrid(image, dimension, transform);
}

Fallible<int> Detector::computeDimension(Ref<ResultPoint> topLeft, Ref<ResultPoint> topRight, Ref<ResultPoint> bottomLeft,
  ...
  case 2:
    ...
    break;
  case 3:
    std::string s("Bad dimension: " + std::to_string(dimension));
...
float Detector::calculateModuleSizeOneWay(Ref<ResultPoint> pattern, Ref<ResultPoint> otherPattern) {
  ...
  if (cvIsNaN(moduleSizeEst1)) {
    return moduleSizeEst2;
  }
  if (cvIsNaN(moduleSizeEst2)) {
...
FallibleRef<AlignmentPattern> Detector::findAlignmentInRegion(float overallEstModuleSize, int estAlignmentX, int estAlignmentY,
  ...
  if ((float) (alignmentAreaRightX - alignmentAreaLeftX) < overallEstModuleSize * 3.f) {
      return failure<ReaderException>("Region too small to hold alignment pattern");
  }
  int alignmentAreaTopY = max(0, estAlignmentY - allowance);
  int alignmentAreaBottomY = min((int)(image_->getHeight() - 1), estAlignmentY + allowance);

  if ((float) (alignmentAreaBottomY - alignmentAreaTopY) < overallEstModuleSize * 3.f) {
      return failure<ReaderException>("Region too small to hold alignment pattern");
  }
