...
#include "zxing/qrcode/decoder/ZXingQRCodeDataBlock.h"
#include "zxing/ReaderException.h"
#include "zxing/qrcode/ZXingQRCodeVersion.h"        // for ECB, ECBlocks, Version
...
DataBlock::DataBlock(int numDataCodewords, ArrayRef<unsigned char> codewords) noexcept :
...
Fallible<std::vector<Ref<DataBlock> >> DataBlock::getDataBlocks(ArrayRef<unsigned char> rawCodewords, Version *version,
    ErrorCorrectionLevel &ecLevel) MB_NOEXCEPT_EXCEPT_BADALLOC {
  ...
  while (longerBlocksStartAt >= 0) {
    ...
    if (numCodewords != shorterBlocksTotalCodewords + 1) {
      return failure<ReaderException>("Data block sizes differ by more than 1");
    }
    longerBlocksStartAt--;
  }
  longerBlocksStartAt++;
  ...
  if ((size_t)rawCodewordsOffset != rawCodewords.size()) {
      return failure<ReaderException>("rawCodewordsOffset != rawCodewords.length");
  }
