...
#include "zxing/bigint/BigUnsigned.h"  // for BigUnsigned, BigUnsigned::Blk, BigUnsigned::CmpRes::equal, BigUnsigned::CmpRes::greater, BigUnsigned::CmpRes::less, convertToPrimitive
...
BigInteger::BigInteger(const Blk *b, Index blen, Sign s) : mag(b, blen) {
    switch (s) {
    case zero:
        MB_ASSERTM(mag.isZero(), "%s", "BigInteger::BigInteger(const Blk *, Index, Sign): Cannot use a sign of zero with a nonzero magnitude");
    ...
    default:
        ...
        MB_ASSERTM(false, "%s", "BigInteger::BigInteger(const Blk *, Index, Sign): Invalid sign");
    }
}
...
BigInteger::BigInteger(const BigUnsigned &x, Sign s) : mag(x) {
    switch (s) {
    case zero:
        MB_ASSERTM(mag.isZero(), "%s", "BigInteger::BigInteger(const BigUnsigned &, Sign): Cannot use a sign of zero with a nonzero magnitude");
    ...
    default:
        /* g++ seems to be optimizing out this case on the assumption
         * that the sign is a valid member of the enumeration.  Oh well. */
        MB_ASSERTM(false, "%s", "BigInteger::BigInteger(const BigUnsigned &, Sign): Invalid sign");
    }
}
...
X convertToUnsignedPrimitive(const BigInteger&bi) {
    ...
    MB_ASSERTM(bi.getSign() != BigInteger::negative, "%s", "BigInteger::to<Primitive>: Cannot convert a negative integer to an unsigned type");
...
X convertToSignedPrimitive(const BigInteger&bi) {
    ...
    MB_ASSERTM(false, "%s", "BigInteger::to<Primitive>: Value is too big to fit in the requested type");
    return 0;
}
...
BigInteger::CmpRes BigInteger::compareTo(const BigInteger &x) const {
    ...
    default:
        MB_ASSERTM(false, "%s", "BigInteger internal error");
        BOOST_ASSUME( false );
    }
}
...
void BigInteger::divideWithRemainder(const BigInteger &b, BigInteger &q) {
    ...
    MB_ASSERTM(this != &q, "%s", "BigInteger::divideWithRemainder: Cannot write quotient and remainder into the same variable");
