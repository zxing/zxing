...
#include "NumberlikeArray.h"  // for NumberlikeArray, NumberlikeArray<>::Index, NumberlikeArray<>::N
...
protected:
    ...
    template <class X> void initFromSignedPrimitive(X x){
        ...
        MB_ASSERTM(x >= 0, "%s", "BigUnsigned constructor: Cannot construct a BigUnsigned from a negative number");
...
inline BigUnsigned BigUnsigned::operator /(const BigUnsigned &x) const {
    MB_ASSERTM(!x.isZero(), "%s", "BigUnsigned::operator /: division by zero");
...
X convertToPrimitive(const BigUnsigned&bu) {
    ...
    MB_ASSERTM(false, "%s", "BigUnsigned::to<Primitive>: Value is too big to fit in the requested type");
    return 0;
}
...
X convertToSignedPrimitiveUI(const BigUnsigned &bu) {
    ...
    MB_ASSERTM(x >= 0, "%s", "BigUnsigned::to(Primitive): Value is too big to fit in the requested type");
