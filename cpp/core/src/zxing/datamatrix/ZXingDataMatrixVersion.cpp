...
#include <zxing/datamatrix/ZXingDataMatrixVersion.h>
...
FallibleRef<Version> Version::getVersionForDimensions(int numRows, int numColumns) MB_NOEXCEPT_EXCEPT_BADALLOC {
    if ((numRows & 0x01) != 0 || (numColumns & 0x01) != 0) {
      return failure<ReaderException>("Number of rows and columns must be even");
    }
    ...
    for (int i = 0; i < N_VERSIONS; ++i){
      ...
      if (version && version->getSymbolSizeRows() == numRows && version->getSymbolSizeColumns() == numColumns) {
        return version;
      }
    }
    return failure<ReaderException>("Error version not found");
  }
