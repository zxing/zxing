...
#include <zxing/datamatrix/detector/ZXingDataMatrixDetector.h>
...
FallibleRef<DetectorResult> Detector::detect() {
  auto const tryCreateRectangle(WhiteRectangleDetector::createWhiteRectangleDetector(image_));
  if(!tryCreateRectangle)
      tryCreateRectangle.error();

  Ref<WhiteRectangleDetector> rectangleDetector_(*tryCreateRectangle);

  auto const tryDetect(rectangleDetector_->detect());
  if(!tryDetect)
      return tryDetect.error();

  std::vector<Ref<ResultPoint> > ResultPoints = *tryDetect;
  ...
  if (4 * dimensionTop >= 7 * dimensionRight || 4 * dimensionRight >= 7 * dimensionTop) {
    ...
    auto const trySamplingGrid(sampleGrid(image_, dimensionTop, dimensionRight, transform));
    if(!trySamplingGrid)
        return trySamplingGrid.error();

    bits = *trySamplingGrid;
    ...
    auto const trySamplingGrid(sampleGrid(image_, dimensionCorrected, dimensionCorrected, transform));
    if(!trySamplingGrid)
        return trySamplingGrid.error();
...
bool Detector::isValid(Ref<ResultPoint> p) {
  return p->getX() >= 0 && p->getX() < static_cast<float>(image_->getWidth()) && p->getY() > 0
      && p->getY() < static_cast<float>(image_->getHeight());
}
...
FallibleRef<BitMatrix> Detector::sampleGrid(Ref<BitMatrix> image, int dimensionX, int dimensionY,
    Ref<PerspectiveTransform> transform) MB_NOEXCEPT_EXCEPT_BADALLOC {
