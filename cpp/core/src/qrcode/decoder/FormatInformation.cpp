...
namespace qrcode {
  namespace decoder {
    ...
    int FormatInformation::BITS_SET_IN_HALF_BYTE[] = { 
    ...
    Ref<FormatInformation> 
    FormatInformation::decodeFormatInformation(int rawFormatInfo) {
    ...
    Ref<FormatInformation> 
    FormatInformation::doDecodeFormatInformation(int rawFormatInfo) {
      // Unmask:
    ...
    bool operator==(const FormatInformation &a,
                    const FormatInformation &b) {
      return &(a.errorCorrectionLevel_) == &(b.errorCorrectionLevel_) &&
      a.dataMask_ == b.dataMask_;
    }
    
    ostream& operator<<(ostream& out, const FormatInformation& fi) {
      const FormatInformation *fip = &fi;
      out << "FormatInformation @ " << (unsigned int)fip;
      return out;
    }
  }
}
