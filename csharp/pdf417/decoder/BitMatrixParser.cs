	...
	sealed class BitMatrixParser
	{
		...
		internal int processRow(int[] rowCounters, int rowNumber, int rowHeight, int[] codewords, int next)
		{
			...
			for (int i = 0; i < width; i += MODULES_IN_SYMBOL)
			{
				...
				{
					if (rowCounters[i + (MODULES_IN_SYMBOL - 1 - mask)] >= SupportClass.URShift(rowHeight, 1))
					{
						symbol |= 1L << mask;
					}
				}
		...
		/// <summary> Build a symbol from the pixels.
		...
		/// 
		/// </summary>
		/// <param name="counters"> array of pixel counter corresponding to each Bar/Space pattern.
		/// </param>
		/// <returns> the symbol
		/// </returns>
		/*
		...
		*/
		...
		private static int findCodewordIndex(long symbol)
		{
			...
			while (first < upto)
			{
				int mid = SupportClass.URShift((first + upto), 1); // Compute mid point.
		...
		*/
		...
		/// <param name="rowHeight">  the height of this row in pixels.
		/// </param>
		/// <param name="moduleWidth">the size of a module in pixels.
		/// </param>
		/// <param name="codewords">  the codeword array to save codewords into.
		/// </param>
		/// <param name="next">       the next available index into the codewords array.
