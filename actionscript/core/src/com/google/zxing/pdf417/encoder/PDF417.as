...
public class PDF417 {
   ...
   */
  private static var START_PATTERN:int = 0x1fea8;
  /**
   * The stop pattern (18 bits)
   */
  private static var STOP_PATTERN:int = 0x3fa29;

  /**
   * The codeword table from the Annex A of ISO/IEC 15438:2001(E).
   */
  private static var CODEWORD_TABLE:Array = [
  ...
  private static var MIN_COLS:int = 2;
  private static var MAX_COLS:int = 30;
  private static var MAX_ROWS:int = 30;
  private static var MIN_ROWS:int = 2;
  private static var DEFAULT_MODULE_WIDTH:Number = 0.357; //1px in mm
  private static var HEIGHT:Number = 16.0; //mm

  private var errorCorrectionLevel:int;
  private var barcodeMatrix:BarcodeMatrix;

  public function getBarcodeMatrix():BarcodeMatrix {
   ...
   */
  private static function getNumberOfRows(m:int, k:int, c:int):int {
    var r:int = calculateNumberOfRows(m, k, c);
    if (r > 90) {
   ...
   * @return the number of rows in the symbol (r)
   */
  private static function calculateNumberOfRows(m:int, k:int, c:int):int {
    var r:int = int((m + 1 + k) / c) + 1;
    if (c * r >= (m + 1 + k + c)) {
      r--;
    }
    return r;
   ...
   */
  private static function getNumberOfPadCodewords(m:int, k:int, c:int, r:int):int {
    var n:int = c * r - k;
   ...
   */
  private static function getNumberOfDataCodewords(m:int, errorCorrectionLevel:int, c:int):int {
    var k:int = PDF417ErrorCorrection.getErrorCorrectionCodewordCount(errorCorrectionLevel);
    var r:int = getNumberOfRows(m, k, c);
    return c * r - k;
  }

  private static function encodeChar(pattern:int, len:int, logic:BarcodeRow):void {
    var map:int = 1 << len - 1;
    var last:Boolean = (pattern & map) != 0; //Initialize to inverse of first bit
    var width:int = 0;
    for (var i:int = 0; i < len; i++) {
      var black:Boolean = (pattern & map) != 0;
      if (last == black) {
        width++;
      } else {
  ...
  private function  encodeLowLevel(fullCodewords:String, c:int, r:int, errorCorrectionLevel:int, logic:BarcodeMatrix ):void {

    this.errorCorrectionLevel = errorCorrectionLevel;

    var idx:int = 0;
    for (var y:int = 0; y < r; y++) {
      var cluster:int = y % 3;
      logic.startRow();
      var bcr:BarcodeRow = logic.getCurrentRow();
      encodeChar(START_PATTERN, 17, bcr );

      var left:int;
      var right:int;
      if (cluster == 0) {
        left = (30 * int(y / 3)) + int((r - 1) / 3);
        right = (30 * int(y / 3)) + (c - 1);
      } else if (cluster == 1) {
        left = (30 * int(y / 3)) + (errorCorrectionLevel * 3) + int((r - 1) % 3);
        right = (30 * int(y / 3)) + int((r - 1) / 3);
      } else {
        left = (30 * int(y / 3)) + (c - 1);
        right = (30 * int(y / 3)) + (errorCorrectionLevel * 3) + int((r - 1) % 3);
      }

      var pattern:int = CODEWORD_TABLE[cluster][left];
      encodeChar(pattern, 17, logic.getCurrentRow());

      for (var x:int = 0; x < c; x++) {
        pattern = CODEWORD_TABLE[cluster][fullCodewords.charCodeAt(idx)];
        encodeChar(pattern, 17, logic.getCurrentRow());
        idx++;
      }
   ...
   */
  public function generateBarcodeLogic(msg:String, errorCorrectionLevel:int):void {

    //1. step: High-level encoding
    var errorCorrectionCodeWords:int = PDF417ErrorCorrection.getErrorCorrectionCodewordCount(errorCorrectionLevel);

	// actionscript does not allow static code parts so we need an instance of the PDF417HighLevelEncoder
    var phe:PDF417HighLevelEncoder = new PDF417HighLevelEncoder();
    var highLevel:String = phe.encodeHighLevel(msg);
    var sourceCodeWords:int = highLevel.length;

    var dimension:Array = determineDimensions(sourceCodeWords);

    if (dimension == null) {
      throw new WriterException("Unable to fit message in columns");
    }

    var cols:int = dimension[0];
    var rows:int = dimension[1];

    var pad:int = getNumberOfPadCodewords(sourceCodeWords, errorCorrectionCodeWords, cols, rows);

    //2. step: construct data codewords
    var n:int = getNumberOfDataCodewords(sourceCodeWords, errorCorrectionLevel, cols);
    if (n > 929) {
      throw new WriterException(
          "Encoded message contains to many code words, message to big (" + msg.length + " bytes)");
    }

    var sb:StringBuilder  = new StringBuilder(n);
    sb.Append(String.fromCharCode(n));
    sb.Append(highLevel);
    for (var i:int = 0; i < pad; i++) {
      sb.Append("900"); //PAD characters
    }
    var dataCodewords:String = sb.toString();
    

    //3. step: Error correction
    var ec:String = PDF417ErrorCorrection.generateErrorCorrection(dataCodewords, errorCorrectionLevel);
    var fullCodewords:String = dataCodewords + ec;
   ...
   */
  public function determineDimensions(sourceCodeWords:int):Array {

    var ratio:Number = 0.0;
    var dimension:Array = null;
    var errorCorrectionCodeWords:int = PDF417ErrorCorrection.getErrorCorrectionCodewordCount(errorCorrectionLevel);

    for (var cols:int = MIN_COLS; cols <= MAX_COLS; cols++) {

      var rows:int = calculateNumberOfRows(sourceCodeWords, errorCorrectionCodeWords, cols);
      ...
      var newRatio:Number = ((17 * cols + 69) * DEFAULT_MODULE_WIDTH) / (rows * HEIGHT);

      // ignore if previous ratio is closer to preferred ratio
      var preferredRatio:Number = 3.0;
