...
public class PDF417ErrorCorrection {
   ...
   */
  private static var EC_COEFFICIENTS:Array = [
      [27, 917],
      [522, 568, 723, 809],
  ...
  public function PDF417ErrorCorrection():void {
  }
   ...
   */
  public static function getErrorCorrectionCodewordCount(errorCorrectionLevel:int):int {
   ...
   */
  public static function getRecommendedMinimumErrorCorrectionLevel(n:int):int {
    if (n <= 0) {
   ...
   */
  public static function generateErrorCorrection(dataCodewords:String, errorCorrectionLevel:int):String {
    var k:int = getErrorCorrectionCodewordCount(errorCorrectionLevel);
    var e:Array = new Array(k);for(var i1:int=0;i1<k;i1++){e[i1] = 0;}
    var sld:int = dataCodewords.length;
    for (var i:int = 0; i < sld; i++) 
    {
      var t1:int = (dataCodewords.charCodeAt(i) + e[e.length - 1]) % 929;
      var t2:int;
      var t3:int;
      for (j = k - 1; j >= 1; j--) 
      {
    ...
    var sb:StringBuilder  = new StringBuilder(k);
    for (var j:int = k - 1; j >= 0; j--) {
      if (e[j] != 0) 
      {
        e[j] = 929 - (e[j]);        
      }
      sb.Append(String.fromCharCode(e[j]));
    }
    return sb.ToString();
  }
}
