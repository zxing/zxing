...
import com.google.zxing.common.BitMatrix;
  ...
  private static var MAX_ROW_DIFFERENCE:int = 6;
  private static var MAX_ROWS:int = 90;
  //private static final int MAX_COLUMNS = 30;
  // Maximum Codewords (Data + Error)
  private static var MAX_CW_CAPACITY:int = 929;
  private static var MODULES_IN_SYMBOL:int = 17;

  private var bitMatrix:BitMatrix;
  private var rows:int = 0;
  //private int columns = 0;

  private var leftColumnECData:int = 0;
  private var rightColumnECData:int = 0;
  private var eraseCount:int = 0;
  private var erasures:Array = null;
  private var ecLevel:int = -1;
  ...
  public function readCodewords():Array 
  {
    ...
    for (var i:int = 1; i < height; i++) {
      ...
      for (var j:int = 0; j < width; j++) {
        ...
        // previous line.
        if (bitMatrix._get(j, i) != bitMatrix._get(j, i - 1)) {
          rowDifference++;
        }
      }
      if (rowDifference <= moduleWidth * MAX_ROW_DIFFERENCE) {
        for (var j2:int = 0; j2 < width; j2++) {
        ...
        if (rowInProgress) {
          ...
          for (var j3:int = 0; j3 < rowCounters.length; j3++) {
            rowCounters[j3] = 0;
          }
          rowNumber++;
          rowHeight = 0;
   ...
   */
  private static function trimArray(array:Array, size:int):Array {
    if (size > 0) {
  ...
  public function processRow(rowCounters:Array, rowNumber:int, rowHeight:int, codewords:Array, next:int):int {
    ...
    {
      for (var mask:int = MODULES_IN_SYMBOL - 1; mask >= 0; mask--) 
      {
   ...
   */
  private static function getCodeword(symbol:Number):int {
    var sym:Number = symbol;
    sym &= 0x3ffff;
    var i:int = findCodewordIndex(sym);
    if (i == -1) {
      return -1;
    } else {
      var cw:Number = CODEWORD_TABLE[i] - 1;
      cw %= 929;
      return int( cw);
    }
  }
   ...
   */
  private static function findCodewordIndex(symbol:Number):int {
    var first:int = 0;
    var upto:int = SYMBOL_TABLE.length;
    while (first < upto) {
      var mid:int = (first + upto) >>> 1; // Compute mid point.
   ...
   */
  public function getErasures():Array {
    return erasures;
  }

  public function getECLevel():int {
    return ecLevel;
  }
   ...
   */
  private static var SYMBOL_TABLE:Array = [0x1025e, 0x1027a, 0x1029e,
   ...
   */
  private static var CODEWORD_TABLE:Array = [2627, 1819, 2622, 2621, 1813,
