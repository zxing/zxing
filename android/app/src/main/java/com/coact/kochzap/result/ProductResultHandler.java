...
public final class ProductResultHandler extends ResultHandler {
  ...
  private static String getProductIDFromResult(ParsedResult rawResult) {
    if (rawResult instanceof ProductParsedResult) {
      String prodID =  ((ProductParsedResult) rawResult).getNormalizedProductID();

      if (Companies.containsscan(prodID)) {
        // On the boycott list
        Toast.makeText(theActivity.getApplicationContext(),
                "*** STOP *** FOUND " + prodID + " AND IT MATCHES, DO NOT BUY IT! ***",
                Toast.LENGTH_SHORT).show();

      } else {
        Toast.makeText(theActivity.getApplicationContext(),
                "Checked " + prodID + " and found no match, go ahead.",
                Toast.LENGTH_SHORT).show();
      }
